...
void SQLog(NSString *format, ...) {
#ifdef DEBUG
    if (![SQNetworkConfig sharedConfig].debugLogEnabled) {
        return;
    }
    va_list argptr;
...
+ (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        SQLog(@"error to set do not backup attribute, error = %@", error);
    }
}
...
@implementation SQRequest (RequestAccessory)
