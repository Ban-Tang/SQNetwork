...
#define kSQNetworkIncompleteDownloadFolderName @"Incomplete"
...
@implementation SQNetworkAgent {
    ...
    NSMutableDictionary<NSNumber *, SQRequest *> *_requestsRecord;
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _config = [SQNetworkConfig sharedConfig];
        _manager = [[AFHTTPSessionManager alloc] initWithSessionConfiguration:_config.sessionConfiguration];
        _requestsRecord = [NSMutableDictionary dictionary];
        _processingQueue = dispatch_queue_create("com.bantang.networkagent.processing", DISPATCH_QUEUE_CONCURRENT);
...
- (NSString *)buildRequestUrl:(__kindof SQRequest *)request {
    ...
    id<SQUrlFilterProtocol> filter = [_config urlFilter];
    if (filter) {
...
- (AFHTTPRequestSerializer *)requestSerializerForRequest:(SQRequest *)request {
    AFHTTPRequestSerializer *requestSerializer = nil;
    if (request.child.requestSerializerType == SQRequestSerializerTypeHTTP) {
        requestSerializer = [AFHTTPRequestSerializer serializer];
    } else if (request.child.requestSerializerType == SQRequestSerializerTypeJSON) {
...
- (NSURLSessionTask *)sessionTaskForRequest:(SQRequest *)request error:(NSError * _Nullable __autoreleasing *)error {
    SQRequestMethod method = [request.child requestMethod];
...
- (void)addRequest:(__kindof SQRequest *)request {
    ...
    // Retain request
    SQLog(@"Add request: %@", NSStringFromClass([request class]));
...
- (void)cancelAllRequests {
    ...
    if (allKeys && allKeys.count > 0) {
        ...
        for (NSNumber *key in copiedKeys) {
            Lock();
            SQRequest *request = _requestsRecord[key];
            Unlock();
...
- (BOOL)validateResult:(SQRequest *)request error:(NSError * _Nullable __autoreleasing *)error {
    BOOL result = [request.child statusCodeValidator];
    if (!result) {
        if (error) {
            *error = [NSError errorWithDomain:SQRequestValidationErrorDomain code:SQRequestValidationErrorInvalidStatusCode userInfo:@{NSLocalizedDescriptionKey:@"Invalid status code"}];
        }
        return result;
    }
    ...
    if (json && validator) {
        result = [SQNetworkUtils validateJSON:json withValidator:validator];
        if (!result) {
            if (error) {
                *error = [NSError errorWithDomain:SQRequestValidationErrorDomain code:SQRequestValidationErrorInvalidJSONFormat userInfo:@{NSLocalizedDescriptionKey:@"Invalid JSON format"}];
            }
            return result;
        }
    }
...
- (void)handleRequestResult:(NSURLSessionTask *)task responseObject:(id)responseObject error:(NSError *)error {
    Lock();
    SQRequest *request = _requestsRecord[@(task.taskIdentifier)];
    Unlock();
    ...
    SQLog(@"SQRequest Finished Request: %@", request);
    ...
    if ([request.responseObject isKindOfClass:[NSData class]]) {
        ...
        request.responseString = [[NSString alloc] initWithData:responseObject encoding:[SQNetworkUtils stringEncodingWithRequest:request]];
...
- (void)requestDidFailWithRequest:(SQRequest *)request error:(NSError *)error {
    request.error = error;
    SQLog(@"SQRequest %@ failed, status code = %ld, error = %@",
    ...
    if ([request.responseObject isKindOfClass:[NSURL class]]) {
        ...
        if (url.isFileURL && [[NSFileManager defaultManager] fileExistsAtPath:url.path]) {
            ...
            request.responseString = [[NSString alloc] initWithData:request.responseData encoding:[SQNetworkUtils stringEncodingWithRequest:request]];
...
- (void)addRequestToRecord:(SQRequest *)request {
    Lock();
    _requestsRecord[@(request.requestTask.taskIdentifier)] = request;
    Unlock();
}

- (void)removeRequestFromRecord:(SQRequest *)request {
    Lock();
    [_requestsRecord removeObjectForKey:@(request.requestTask.taskIdentifier)];
    SQLog(@"Request queue size = %zd", [_requestsRecord count]);
    Unlock();
}
...
- (NSURLSessionDownloadTask *)downloadTaskWithDownloadPath:(NSString *)downloadPath
    ...
    BOOL resumeDataIsValid = [SQNetworkUtils validateResumeData:data];
    ...
    if (canBeResumed) {
        @try {
            ...
            SQLog(@"Resume download failed, reason = %@", exception.reason);
            resumeSucceeded = NO;
        }
    }
    if (!resumeSucceeded) {
...
- (NSString *)incompleteDownloadTempCacheFolder {
    ...
    if (!cacheFolder) {
        ...
        cacheFolder = [cacheDir stringByAppendingPathComponent:kSQNetworkIncompleteDownloadFolderName];
    }
    
    NSError *error = nil;
    if(![fileManager createDirectoryAtPath:cacheFolder withIntermediateDirectories:YES attributes:nil error:&error]) {
        SQLog(@"Failed to create cache directory at %@", cacheFolder);
        cacheFolder = nil;
    }
    return cacheFolder;
}
...
- (NSURL *)incompleteDownloadTempPathForDownloadPath:(NSString *)downloadPath {
    ...
    NSString *md5URLString = [SQNetworkUtils md5StringFromString:downloadPath];
